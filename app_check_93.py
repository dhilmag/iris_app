...
def prediction(model, SepalLength, SepalWidth, PetalLength, PetalWidth):
  ...
  species = species[0]
  if species == 0:
    return "Your flower is Iris-setosa"
  elif species == 1:
    return "Your flower is Iris-virginica"
  else:
    return "Your flower is Iris-versicolor"
...
s_length = st.sidebar.slider("Choose Sepal Length", 0, 10)
s_width = st.sidebar.slider("Choose Sepal Width", 0.0, 10.0)
p_length = st.sidebar.slider("Choose Petal Length", 0.0, 10.0)
p_width = st.sidebar.slider("Choose Petal Width", 0.0, 10.0)
